module "ec2_backend_main" {
  ...
  iam_role_description        = "IAM role for EC2 instance: ${var.env}-${data.terraform_remote_state.networking.outputs.ec2_backend_main_name}"
  iam_role_policies = {
...
resource "aws_lb_target_group_attachment" "ec2_backend_main" {
  target_group_arn = aws_lb_target_group.backend.arn
  target_id        = module.ec2_backend_main.id
  port             = 5000
}