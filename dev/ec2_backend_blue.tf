module "ec2_backend_blue" {
  source  = "terraform-aws-modules/ec2-instance/aws"
  version = "5.7.0"

  name = "${var.env}-${data.terraform_remote_state.networking.outputs.ec2_backend_blue_name}"
  ...
  iam_role_description        = "IAM role for EC2 instance: ${var.env}-${data.terraform_remote_state.networking.outputs.ec2_backend_blue_name}"
  iam_role_policies = {
...
resource "aws_lb_target_group_attachment" "ec2_backend_blue" {
  target_group_arn = aws_lb_target_group.backend_blue.arn
  target_id        = module.ec2_backend_blue.id
  port             = 5000
}